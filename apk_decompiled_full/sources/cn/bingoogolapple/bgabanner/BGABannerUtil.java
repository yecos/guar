...
public class BGABannerUtil {
    ...
    */
    public static Bitmap getScaledImageFromResource(Context context, int i10, int i11, int i12, float f10, float f11) {
        LoadBitmapPair<Throwable> imageFromResource;
        do {
            imageFromResource = getImageFromResource(context, i10, i11, i12);
            if (imageFromResource != null && imageFromResource.first != null) {
                break;
            }
            i11 /= 2;
            i12 /= 2;
            if (imageFromResource == null || !(imageFromResource.second instanceof OutOfMemoryError) || i11 <= f10) {
                break;
            }
        } while (i12 > f11);

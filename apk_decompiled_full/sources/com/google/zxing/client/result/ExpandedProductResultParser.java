package com.google.zxing.client.result;

import com.google.zxing.BarcodeFormat;
import com.google.zxing.Result;
import com.hpplay.component.protocol.plist.ASCIIPropertyListParser;
import com.hpplay.sdk.source.utils.ErrorCode;
import java.util.HashMap;
import org.android.agoo.common.AgooConstants;

/* loaded from: classes2.dex */
public final class ExpandedProductResultParser extends ResultParser {
    private static String findAIvalue(int i10, String str) {
        if (str.charAt(i10) != '(') {
            return null;
        }
        String substring = str.substring(i10 + 1);
        StringBuilder sb = new StringBuilder();
        for (int i11 = 0; i11 < substring.length(); i11++) {
            char charAt = substring.charAt(i11);
            if (charAt == ')') {
                return sb.toString();
            }
            if (charAt < '0' || charAt > '9') {
                return null;
            }
            sb.append(charAt);
        }
        return sb.toString();
    }

    private static String findValue(int i10, String str) {
        StringBuilder sb = new StringBuilder();
        String substring = str.substring(i10);
        for (int i11 = 0; i11 < substring.length(); i11++) {
            char charAt = substring.charAt(i11);
            if (charAt != '(') {
                sb.append(charAt);
            } else {
                if (findAIvalue(i11, substring) != null) {
                    break;
                }
                sb.append(ASCIIPropertyListParser.ARRAY_BEGIN_TOKEN);
            }
        }
        return sb.toString();
    }

    /* JADX WARN: Can't fix incorrect switch cases order, some code will duplicate */
    /* JADX WARN: Code restructure failed: missing block: B:144:0x020b, code lost:
    
        if (r1.equals(org.android.agoo.common.AgooConstants.ACK_REMOVE_PACKAGE) == false) goto L13;
     */
    @Override // com.google.zxing.client.result.ResultParser
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public ExpandedProductParsedResult parse(Result result) {
        ExpandedProductParsedResult expandedProductParsedResult = null;
        if (result.getBarcodeFormat() != BarcodeFormat.RSS_EXPANDED) {
            return null;
        }
        String massagedText = ResultParser.getMassagedText(result);
        HashMap hashMap = new HashMap();
        String str = null;
        String str2 = null;
        String str3 = null;
        String str4 = null;
        String str5 = null;
        String str6 = null;
        String str7 = null;
        String str8 = null;
        String str9 = null;
        String str10 = null;
        String str11 = null;
        String str12 = null;
        String str13 = null;
        int i10 = 0;
        while (i10 < massagedText.length()) {
            String findAIvalue = findAIvalue(i10, massagedText);
            if (findAIvalue == null) {
                return expandedProductParsedResult;
            }
            char c10 = 2;
            int length = i10 + findAIvalue.length() + 2;
            String findValue = findValue(length, massagedText);
            int length2 = length + findValue.length();
            switch (findAIvalue.hashCode()) {
                case 1536:
                    if (findAIvalue.equals("00")) {
                        c10 = 0;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1537:
                    if (findAIvalue.equals(ErrorCode.MODULE_BROWSE)) {
                        c10 = 1;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567:
                    break;
                case 1568:
                    if (findAIvalue.equals(AgooConstants.ACK_BODY_NULL)) {
                        c10 = 3;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1570:
                    if (findAIvalue.equals(AgooConstants.ACK_FLAG_NULL)) {
                        c10 = 4;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1572:
                    if (findAIvalue.equals(AgooConstants.ACK_PACK_ERROR)) {
                        c10 = 5;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1574:
                    if (findAIvalue.equals("17")) {
                        c10 = 6;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567966:
                    if (findAIvalue.equals("3100")) {
                        c10 = 7;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567967:
                    if (findAIvalue.equals("3101")) {
                        c10 = '\b';
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567968:
                    if (findAIvalue.equals("3102")) {
                        c10 = '\t';
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567969:
                    if (findAIvalue.equals("3103")) {
                        c10 = '\n';
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567970:
                    if (findAIvalue.equals("3104")) {
                        c10 = 11;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567971:
                    if (findAIvalue.equals("3105")) {
                        c10 = '\f';
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567972:
                    if (findAIvalue.equals("3106")) {
                        c10 = ASCIIPropertyListParser.WHITESPACE_CARRIAGE_RETURN;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567973:
                    if (findAIvalue.equals("3107")) {
                        c10 = 14;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567974:
                    if (findAIvalue.equals("3108")) {
                        c10 = 15;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1567975:
                    if (findAIvalue.equals("3109")) {
                        c10 = 16;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568927:
                    if (findAIvalue.equals("3200")) {
                        c10 = 17;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568928:
                    if (findAIvalue.equals("3201")) {
                        c10 = 18;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568929:
                    if (findAIvalue.equals("3202")) {
                        c10 = 19;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568930:
                    if (findAIvalue.equals("3203")) {
                        c10 = 20;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568931:
                    if (findAIvalue.equals("3204")) {
                        c10 = 21;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568932:
                    if (findAIvalue.equals("3205")) {
                        c10 = 22;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568933:
                    if (findAIvalue.equals("3206")) {
                        c10 = 23;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568934:
                    if (findAIvalue.equals("3207")) {
                        c10 = 24;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568935:
                    if (findAIvalue.equals("3208")) {
                        c10 = 25;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1568936:
                    if (findAIvalue.equals("3209")) {
                        c10 = 26;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575716:
                    if (findAIvalue.equals("3920")) {
                        c10 = 27;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575717:
                    if (findAIvalue.equals("3921")) {
                        c10 = 28;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575718:
                    if (findAIvalue.equals("3922")) {
                        c10 = 29;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575719:
                    if (findAIvalue.equals("3923")) {
                        c10 = 30;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575747:
                    if (findAIvalue.equals("3930")) {
                        c10 = 31;
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575748:
                    if (findAIvalue.equals("3931")) {
                        c10 = ' ';
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575749:
                    if (findAIvalue.equals("3932")) {
                        c10 = '!';
                        break;
                    }
                    c10 = 65535;
                    break;
                case 1575750:
                    if (findAIvalue.equals("3933")) {
                        c10 = '\"';
                        break;
                    }
                    c10 = 65535;
                    break;
                default:
                    c10 = 65535;
                    break;
            }
            switch (c10) {
                case 0:
                    str2 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 1:
                    str = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 2:
                    str3 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 3:
                    str4 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 4:
                    str5 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 5:
                    str6 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 6:
                    str7 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 7:
                case '\b':
                case '\t':
                case '\n':
                case 11:
                case '\f':
                case '\r':
                case 14:
                case 15:
                case 16:
                    str10 = findAIvalue.substring(3);
                    str9 = ExpandedProductParsedResult.KILOGRAM;
                    str8 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 17:
                case 18:
                case 19:
                case 20:
                case 21:
                case 22:
                case 23:
                case 24:
                case 25:
                case 26:
                    str10 = findAIvalue.substring(3);
                    str9 = ExpandedProductParsedResult.POUND;
                    str8 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 27:
                case 28:
                case 29:
                case 30:
                    str12 = findAIvalue.substring(3);
                    str11 = findValue;
                    i10 = length2;
                    expandedProductParsedResult = null;
                case 31:
                case ' ':
                case '!':
                case '\"':
                    if (findValue.length() < 4) {
                        return null;
                    }
                    String substring = findValue.substring(3);
                    str13 = findValue.substring(0, 3);
                    str11 = substring;
                    i10 = length2;
                    expandedProductParsedResult = null;
                    str12 = findAIvalue.substring(3);
                default:
                    hashMap.put(findAIvalue, findValue);
                    i10 = length2;
                    expandedProductParsedResult = null;
            }
        }
        return new ExpandedProductParsedResult(massagedText, str, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11, str12, str13, hashMap);
    }
}

...
public class LelinkReverseChannel extends ProtocolCore implements Runnable {
    ...
    */
    public void parseResponse(byte[] bArr) {
        ...
        try {
            byte[] decryptData = this.mLelinkEncrypt.decryptData(bArr);
            if (decryptData == null || decryptData.length <= 0) {
                return;
            }
            byte[] body = ProtocolUtils.getBody(decryptData);
            if (body != null && body.length >= 10) {
                str = new String(body, 0, body.length, XML.CHARSET_UTF8);
                CLog.i(TAG, str);
                protocolListener = this.mProtocolListener;
                if (protocolListener == null) {
                    protocolListener.onResult(protocolListener.cmdType, str);
                    return;
                }
            ...
            str = new String(decryptData, 0, decryptData.length, XML.CHARSET_UTF8);
            CLog.i(TAG, str);
            protocolListener = this.mProtocolListener;
            if (protocolListener == null) {
            }

...
public class LelinkPushBridge extends AbsBridge {
    ...
    */
    public void resolveProtocolInfo(int i10, String... strArr) {
        String str;
        String str2;
        SourceLog.i(TAG, "====resolveProtocolInfo cmd " + i10 + "," + Arrays.toString(strArr));
        String str3 = null;
        if (i10 == 1) {
            printResult("CMD_PUSH ", strArr);
            ...
            SourceLog.i(TAG, "push " + strArr[0]);
            if ("successful".equals(strArr[0])) {
                this.mHandler.sendMessage(Message.obtain(null, 1, 1, 0, strArr[1]));
                updatePosition();
                return;
            } else if (!ProtocolBuilder.LELINK_AUTH_ERROR.equals(strArr[0])) {
                this.mHandler.sendMessage(Message.obtain(null, 1, 0, 0, strArr[1]));
                return;
            } else {
        ...
        if (i10 == 8) {
            updateDramaIdOnCallback(i10, strArr);
            notifyPosition(strArr);
            return;
        }
        if (i10 == 9) {
            if (strArr.length >= 1 && !TextUtils.isEmpty(strArr[0])) {
                String str4 = strArr[0];
                if ("playing".equals(str4) && this.mHandler.hasMessages(16)) {
                    SourceLog.w(TAG, "CMD_GET_STATEINFO there has a stop msg,remove it");
                    this.mHandler.removeMessages(16);
                }
                if (TextUtils.equals(this.mDlnaPreState, str4)) {
                    return;
                }
                callbackDlnaStateChanged(str4);
                this.mDlnaPreState = str4;
                return;
            }
            return;
        }
        if (i10 == 26) {
            printResult("EPISODE_STOPED", strArr);
            updateDramaIdOnCallback(i10, strArr);
        ...
        if (i10 != 28) {
            switch (i10) {
                case 11:
                    printResult("CMD_CONNECT ", strArr);
                    break;
                case 12:
                    printResult("CMD_ON_COMPLETION", strArr);
                    updateDramaIdOnCallback(i10, strArr);
                    this.mHandler.removeMessages(12);
                    ...
                    break;
                case 13:
                    printResult("CMD_ON_START_PLAY", strArr);
                    updateDramaIdOnCallback(i10, strArr);
                    try {
                        if (this.mPlayInfo.protocol == 3) {
                            ...
                            try {
                                ...
                                if (TextUtils.isEmpty(this.mDlnaUUID)) {
                                }
                                if (this.mHandler.hasMessages(16)) {
                    ...
                    if (TextUtils.isEmpty(str3) && !isSameUrl(str3, this.mPlayInfo.getPlayUrl())) {
                        SourceLog.w(TAG, "CMD_ON_START_PLAY ignore");
                        if (TextUtils.equals(str3, this.mPreDLNAStopUrl)) {
                        ...
                        if (this.mHandler.hasMessages(16) && (isSameUrl(this.mPlayInfo.getPlayUrl(), str3) || TextUtils.equals(this.mDlnaUUID, str2))) {
                            SourceLog.w(TAG, "there has a stop msg,remove it ");
                            this.mHandler.removeMessages(16);
                        }
                        this.mHandler.sendEmptyMessage(13);
                    ...
                    break;
                case 14:
                    printResult("CMD_ON_LOADING", strArr);
                    updateDramaIdOnCallback(i10, strArr);
                    break;
                case 15:
                    printResult("CMD_ON_PAUSE", strArr);
                    updateDramaIdOnCallback(i10, strArr);
                    this.mHandler.sendEmptyMessage(15);
                    cancelPositionUpdate();
                    break;
        ...
        printResult("CMD_ON_STOPED", strArr);
        if (this.mPlayInfo.protocol == 3) {
            if (!isSameDlna(strArr)) {
                return;
            }
        } else if (!isSameLelink(strArr)) {
            return;
        }
        updateDramaIdOnCallback(i10, strArr);
        notifyStopped(i10 == 28 ? 4 : 1);
    }

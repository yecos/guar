        ...
        AsyncManager.getInstance().exeHttpTask(asyncHttpParameter, new AsyncHttpRequestListener() { // from class: com.hpplay.sdk.source.device.qr.QRCodeController.5
            ...
            */
            public void onRequestResult(AsyncHttpParameter asyncHttpParameter2) {
                ...
                ICreateShortUrlListener iCreateShortUrlListener2;
                ShortUrlBean.DataEntity dataEntity;
                SourceLog.debug("QRCodeController", "requestShortUrl onRequestResult:" + asyncHttpParameter2.out);
                AsyncHttpParameter.Out out = asyncHttpParameter2.out;
                if (out.resultType == 2) {
                ...
                if (out.result != null) {
                    try {
                        shortUrlBean = new ShortUrlBean(new JSONObject(httpEncrypt.decode(out)));
                    } catch (Exception e10) {
                        SourceLog.w("QRCodeController", e10);
                    }
                    if (shortUrlBean != null) {
                        if (shortUrlBean.status != 200 || (dataEntity = shortUrlBean.data) == null || TextUtils.isEmpty(dataEntity.shorturl)) {
                            int i10 = shortUrlBean.status;
                            if (i10 == 401 || i10 == 410) {
                                AuthSDK.getInstance().authSDKByInvalidToken();
                                return;
                            }
                            ...
                            ICreateShortUrlListener iCreateShortUrlListener3 = iCreateShortUrlListener;
                            if (iCreateShortUrlListener3 != null) {
                                iCreateShortUrlListener3.onCreateShortUrl(shortUrlBean.data.shorturl);
                                return;
                            }
                        }
                    }
                    iCreateShortUrlListener2 = iCreateShortUrlListener;
                    if (iCreateShortUrlListener2 == null) {
                        iCreateShortUrlListener2.onCreateShortUrl(null);
                        return;
                    }
                    return;
                }

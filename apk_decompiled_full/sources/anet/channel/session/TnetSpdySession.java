...
public class TnetSpdySession extends Session implements SessionCb {
    ...
    */
    public void connect() {
        ...
        try {
            ...
            ALog.e("awcn.TnetSpdySession", BaseMonitor.ALARM_POINT_CONNECT, this.f3827p, Constants.KEY_HOST, this.f3814c, "ip", this.f3817f, "port", Integer.valueOf(this.f3818g), ParamsMap.DeviceParams.KEY_SESSION_ID, valueOf, "SpdyProtocol,", this.f3821j, "proxyIp,", this.f3819h, "proxyPort,", Integer.valueOf(this.f3820i));
            SessionInfo sessionInfo = new SessionInfo(this.f3817f, this.f3818g, this.f3814c + "_" + this.F, this.f3819h, this.f3820i, valueOf, this, this.f3821j.getTnetConType());
            sessionInfo.setConnectionTimeoutMs((int) (((float) this.f3829r) * Utils.getNetworkTimeFactor()));
            if (!this.f3821j.isPublicKeyAuto() && !this.f3821j.isH2S() && !this.f3821j.isHTTP3()) {
                int i12 = this.B;
                if (i12 >= 0) {
                    sessionInfo.setPubKeySeqNum(i12);
                } else {
                    ConnType connType = this.f3821j;
                ...
                if (this.f3821j.isHTTP3() && (xquicCongControl = AwcnConfig.getXquicCongControl()) >= 0) {
                    sessionInfo.setXquicCongControl(xquicCongControl);
                }
                createSession = this.f4092w.createSession(sessionInfo);
                this.f4093x = createSession;
                if (createSession.getRefCount() <= 1) {
                    ALog.e("awcn.TnetSpdySession", "get session ref count > 1!!!", this.f3827p, new Object[0]);
                ...
                if (TextUtils.isEmpty(this.f3819h)) {
                    i11 = 0;
                }
            ...
            sessionInfo.setCertHost(this.f3824m ? this.f3816e : this.f3815d);
            ...
            notifyStatus(2, null);
            ALog.e("awcn.TnetSpdySession", "connect exception ", this.f3827p, th, new Object[0]);
        }
    }
    ...
    */
    public Cancelable request(Request request, RequestCb requestCb) {
        ...
        requestStatistic.setIPAndPort(this.f3817f, this.f3818g);
        requestStatistic.ipRefer = this.f3822k.getIpSource();
        requestStatistic.ipType = this.f3822k.getIpType();
        requestStatistic.unit = this.f3823l;
        ...
        try {
            if (this.f4093x == null || !((i10 = this.f3825n) == 0 || i10 == 4)) {
                requestCb.onFinish(ErrorConstant.ERROR_SESSION_INVALID, ErrorConstant.getErrMsg(ErrorConstant.ERROR_SESSION_INVALID), request.f4045a);
                return cVar2;
            }
            if (this.f3824m) {
                request.setDnsOptimize(this.f3816e, this.f3818g);
            }
            ...
            try {
                if (!TextUtils.isEmpty(this.f3819h) && this.f3820i > 0) {
                    str = "";
                    spdyRequest = new SpdyRequest(url, url.getHost(), url.getPort(), this.f3819h, this.f3820i, request.getMethod(), RequestPriority.DEFAULT_PRIORITY, -1, request.getConnectTimeout(), 0);
                    spdyRequest.setRequestRdTimeoutMs(request.getReadTimeout());
                    headers = request.getHeaders();
                    if (headers.containsKey("Host")) {
                        spdyRequest.addHeaders(headers);
                        spdyRequest.addHeader(":host", this.f3824m ? this.f3816e : request.getHost());
                    } else {
                    ...
                    request.f4045a.sendStart = System.currentTimeMillis();
                    RequestStatistic requestStatistic2 = request.f4045a;
                    requestStatistic2.processTime = requestStatistic2.sendStart - request.f4045a.start;
                    int submitRequest = this.f4093x.submitRequest(spdyRequest, spdyDataProvider, this, new a(request, requestCb));
                    if (ALog.isPrintLog(1)) {
                        ALog.d("awcn.TnetSpdySession", str, request.getSeq(), "streamId", Integer.valueOf(submitRequest));
                    }
                    cVar = new anet.channel.request.c(this.f4093x, submitRequest, request.getSeq());
                    SessionStatistic sessionStatistic = this.f3828q;
            ...
            SpdyDataProvider spdyDataProvider2 = new SpdyDataProvider(request.getBodyBytes());
            request.f4045a.sendStart = System.currentTimeMillis();
            RequestStatistic requestStatistic22 = request.f4045a;
            requestStatistic22.processTime = requestStatistic22.sendStart - request.f4045a.start;
            int submitRequest2 = this.f4093x.submitRequest(spdyRequest, spdyDataProvider2, this, new a(request, requestCb));
            if (ALog.isPrintLog(1)) {
            }
            cVar = new anet.channel.request.c(this.f4093x, submitRequest2, request.getSeq());
